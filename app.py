...
with left:
    if os.path.exists(LOGO_PATH):
        ...
        st.markdown(f"<div style='font-size:36px;color:{DEFAULT_THEME['primary']};font-weight:800'>SK</div>", unsafe_allow_html=True)
with right:
    st.markdown(f"""
...
# If user loaded JSON mapping, apply if keys look valid
if st.session_state.get("colmap_json"):
    loaded = st.session_state["colmap_json"]
    if isinstance(loaded, dict) and all(k in loaded for k in ["EMP_NAME","ORG","RISK"]):
        # only accept columns that exist
        for k,v in loaded.items():
            if v in df.columns or v == "없음":
                st.session_state["colmap"][k] = v
        st.info("불러온 매핑을 적용했습니다.")
    else:
        st.warning("매핑 JSON 형식이 올바르지 않습니다.")
...
with c1:
    colmap["EMP_NAME"] = st.selectbox("성명", columns, index=columns.index(colmap["EMP_NAME"]) if colmap["EMP_NAME"] in columns else 0, help=col_preview(colmap["EMP_NAME"]))
    st.caption("미리보기: " + col_preview(colmap["EMP_NAME"]))
    colmap["ORG"] = st.selectbox("본부/조직", columns, index=columns.index(colmap["ORG"]) if colmap["ORG"] in columns else 0, help=col_preview(colmap["ORG"]))
    st.caption("미리보기: " + col_preview(colmap["ORG"]))
    colmap["TEAM"] = st.selectbox("팀", columns, index=columns.index(colmap["TEAM"]) if colmap["TEAM"] in columns else 0, help=col_preview(colmap["TEAM"]))
    st.caption("미리보기: " + col_preview(colmap["TEAM"]))
with c2:
    colmap["TALENT"] = st.selectbox("인재등급", columns, index=columns.index(colmap["TALENT"]) if colmap["TALENT"] in columns else 0, help=col_preview(colmap["TALENT"]))
    st.caption("미리보기: " + col_preview(colmap["TALENT"]))
    colmap["GRADE"]  = st.selectbox("성과등급", columns, index=columns.index(colmap["GRADE"])  if colmap["GRADE"]  in columns else 0, help=col_preview(colmap["GRADE"]))
    st.caption("미리보기: " + col_preview(colmap["GRADE"]))
    colmap["LEVEL"]  = st.selectbox("직급레벨", columns, index=columns.index(colmap["LEVEL"])  if colmap["LEVEL"]  in columns else 0, help=col_preview(colmap["LEVEL"]))
    st.caption("미리보기: " + col_preview(colmap["LEVEL"]))
with c3:
    colmap["RISK"]  = st.selectbox("퇴직위험도 (필수)", columns, index=columns.index(colmap["RISK"]) if colmap["RISK"] in columns else 0, help=col_preview(colmap["RISK"]))
    st.caption("미리보기: " + col_preview(colmap["RISK"]))
    colmap["SALARY"]= st.selectbox("연봉(원)", columns, index=columns.index(colmap["SALARY"]) if colmap["SALARY"] in columns else 0, help=col_preview(colmap["SALARY"]))
    st.caption("미리보기: " + col_preview(colmap["SALARY"]))
    colmap["RISK_REASON"] = st.selectbox("위험요인", columns, index=columns.index(colmap["RISK_REASON"]) if colmap["RISK_REASON"] in columns else 0, help=col_preview(colmap["RISK_REASON"]))
    st.caption("미리보기: " + col_preview(colmap["RISK_REASON"]))

c4, c5 = st.columns(2)
with c4:
    colmap["AGE"]    = st.selectbox("나이", columns, index=columns.index(colmap["AGE"]) if colmap["AGE"] in columns else 0, help=col_preview(colmap["AGE"]))
    st.caption("미리보기: " + col_preview(colmap["AGE"]))
    colmap["TENURE"] = st.selectbox("근속연수(년)", columns, index=columns.index(colmap["TENURE"]) if colmap["TENURE"] in columns else 0, help=col_preview(colmap["TENURE"]))
    st.caption("미리보기: " + col_preview(colmap["TENURE"]))
with c5:
    colmap["PHONE"]  = st.selectbox("휴대폰", columns, index=columns.index(colmap["PHONE"]) if colmap["PHONE"] in columns else 0, help=col_preview(colmap["PHONE"]))
    st.caption("미리보기: " + col_preview(colmap["PHONE"]))
    colmap["EMAIL"]  = st.selectbox("이메일", columns, index=columns.index(colmap["EMAIL"]) if colmap["EMAIL"] in columns else 0, help=col_preview(colmap["EMAIL"]))
    st.caption("미리보기: " + col_preview(colmap["EMAIL"]))

# Required checks
missing = [k for k in ["EMP_NAME","ORG","RISK"] if colmap[k] == "없음" or colmap[k] not in df.columns]
if missing:
    st.error("다음 필수 매핑이 누락되었습니다: " + ", ".join(missing))
    st.stop()

# Save mapping JSON
map_json = json.dumps(colmap, ensure_ascii=False, indent=2)
